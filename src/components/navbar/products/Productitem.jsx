...
const Productitem = ({id,name,price,desc,rating,image,offer}) => {
    const dispatch=useDispatch();
    function handlefuntion(){
      toast(name,"added")
    }
  return (
            <div className='box w-[45%] md:w-[31%] min-h-[550px]'>
          ...
          <div className="sub_box relative bg-[whitesmoke] group w-[100%] rounded-[10px] h-[400px] cursor-pointer overflow-hidden">
              ...
              <NavLink to={`/${id}`}>
              <h1 className='text-center md:text-left text-[14px] hover:underline cursor-pointer mb-[10px] md:text-[17px]'>{name}</h1>
              </NavLink>
                <div className='flex md:flex-row flex-col justify-between items-center'>
                <h2 className='text-[15px] pr-[15px]'>${price}<span className='text-paracolor line-through'>$21.00</span></h2>
